...
train$price_gap_ratio<- cut(train$price_gap_ratio,
                       breaks = c(0,0.02, 0.04, 0.06, 0.08, 0.12, 0.2, 0.4, 0.6, 1, 1.5, 2, 20 ), 
                       include.lowest = TRUE,
                       right = TRUE )
...
blist <- data.frame(from = c("is_Amazon" , "price_gap_ratio" , "price_gap_ratio"    ,"sid_pos_fb"  , "rank"    ,"rank"   ), 
                      to = c("price_gap_ratio" ,"is_Amazon" ,  "sid_pos_fb"   , "price_gap_ratio"   ,"price_gap_ratio"  ,"sid_pos_fb"    )  )
...
dag_hc
dag <-model2network("[is_Amazon][price_gap_ratio][sid_pos_fb|is_Amazon][rank|is_Amazon:price_gap_ratio:sid_pos_fb][bbox|is_Amazon:price_gap_ratio:sid_pos_fb:rank]")
graphviz.plot(dag)
