...
train = data %>% select(is_Amazon,price_gap_ratio  ,   bbox )
...
# Hill Climbing
dag_hc = hc(train ,whitelist = wlist )
...
dag_hc
dag <-model2network("[is_Amazon][price_gap_ratio|is_Amazon][bbox|is_Amazon:price_gap_ratio]")
graphviz.plot(dag)
...
testcheck <- XYtrain %>%
  group_by(epoc,pid)%>%
  select(pid,epoc,sid, price_gap_ratio,   is_Amazon)
